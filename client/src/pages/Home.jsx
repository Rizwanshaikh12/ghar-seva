    ...
    useEffect(() => {
        const getEarnings = async () => {
            if (user.role === 'service_provider') {
                const response = await axios.get(`${process.env.REACT_APP_SERVER_BASE_URL}/order/total-earning/${user._id}`, {
                    headers: {
        ...
        const getServices = async () => {
            if (user.role === 'service_provider') {
                const response = await axios.get(`${process.env.REACT_APP_SERVER_BASE_URL}/service/service-provided/${user._id}`, {
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                });
        ...
        const getTestimonials = async () => {
            if (user.role === 'service_provider') {
                const response = await axios.get(`${process.env.REACT_APP_SERVER_BASE_URL}/user/testimonials/${user._id}`, {
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                });
